# C++ Activity: Inheritance

Build complex systems by deriving new classes from existing ones.
...
// Your code here: Create a 'Resident' class that inherits from 'Person'
// and adds an 'int residentID'

int main() {
    // Your code here: Create a Resident object and call both inherited and own methods
...
Topics Covered: Base and Derived Classes, `protected` access, Member Initialization Lists.

1.  **Inherit**: Create a class `Resident` that inherits from the `Person` class using `public` inheritance.
2.  **Add Member**: Add a private or protected member `int residentID` to the `Resident` class.
3.  **Constructor**: Implement a constructor for `Resident` that takes both a name and an ID. Use a member initialization list to pass the name to the `Person` base constructor.
4.  **Display**: Add a method `void showDetails()` to `Resident` that prints both the name (inherited) and the resident ID.
5.  **Test**: In `main`, instantiate a `Resident`, call the inherited `introduce()` method, and the new `showDetails()` method. }
        void showDetails() {
            cout << "Car: " << brand << " (" << year << ") - "
