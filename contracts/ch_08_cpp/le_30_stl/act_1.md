# C++ Activity: Standard Template Library (STL)

Boost your productivity by using built-in, optimized containers and algorithms.
...
int main() {
    // Your code here: Create a vector of strings called 'residents'

    // Your code here: Add at least 5 names to the vector using push_back()

    // Your code here: Sort the names alphabetically using sort()

    // Your code here: Print the sorted names using a range-based for loop
...
Topics Covered: `std::vector`, `std::sort`, Range-based `for` loops, STL Headers.

1. **Create Container**: Declare a `vector<string>` named `residents`.
2. **Add Data**: Use the `push_back()` method to add at least 5 names (e.g., "Juan", "Maria", "Pedro", "Ana", "Lito") to the vector.
3. **Sort**: Use the `std::sort()` algorithm from the `<algorithm>` header to sort the vector in alphabetical order.
4. **Iterate**: Use a range-based `for` loop (e.g., `for (const string& name : residents)`) to print each name on a new line.
5. **Observe**: Note how the STL handles memory allocation (vector) and complex operations (sort) with just a few lines of code.
