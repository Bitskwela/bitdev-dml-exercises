# C++ Activity: Nested Structs

Organize complex data by nesting one struct within another.
...
Topics Covered: Nested Structs, Hierarchical Data, Multi-level dot operator.

1. **Define the Nested Struct**: Create a `struct` called `Date` with three integer members: `day`, `month`, and `year`.
2. **Define the Parent Struct**: Create a `struct` called `Resident` that includes a `string name` and a `Date birthDate`.
3. **Initialize the Data**: In `main`, create a `Resident` and assign values to both the name and the sub-members of `birthDate` (e.g., `res.birthDate.year = 1995`).
4. **Output the Details**: Print the resident's name and their birthday in a `DD/MM/YYYY` format using the nested struct data.}
