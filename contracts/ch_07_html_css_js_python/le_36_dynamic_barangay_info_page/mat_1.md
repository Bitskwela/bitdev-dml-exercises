...
![Cover Image](https://bitdev-dml-assets.s3.ap-southeast-1.amazonaws.com/ch_7/C7+36.0+-+COVER.png)
...
![image](https://bitdev-dml-assets.s3.ap-southeast-1.amazonaws.com/ch_7/C7+36.1.png)
...
function createServiceCard(service) {
  ...
  card.innerHTML = `
        ...
        <span class="service-status ${
          service.available ? "status-available" : "status-unavailable"
        }">
...
```javascript
filteredServices = allServices.filter((service) => {
  return service.name.includes(searchTerm) && matchesStatusFilter(service);
});
```
