# ============================================
# FEATURE ENGINEERING — Full Solution
...
df["payday_rainy"]  = df["is_payday_int"] * df["is_rainy"]
item_dummies = pd.get_dummies(df["item"], prefix="item", drop_first=True)
df_eng = pd.concat([df, item_dummies], axis=1)

eng_cols = ["day_of_month", "is_payday_int", "is_friday", "is_rainy",
            "payday_rainy"] + list(item_dummies.columns)
X_eng = df_eng[eng_cols].to_numpy(dtype=float)
mae_eng, w_eng = fit_predict_mae(X_eng[train_idx], X_eng[test_idx],
                                   y[train_idx], y[test_idx])

print(f"   Engineered ({len(eng_cols)} features): MAE = P{mae_eng:.2f}")
print(f"   Improvement: P{mae_base - mae_eng:.2f} ({(mae_base - mae_eng)/mae_base:.1%})")

print()
print("-- Top weights --")
for name, w in sorted(zip(eng_cols, w_eng[1:]), key=lambda kv: -abs(kv[1]))[:8]:
    print(f"   {name:<22} {w:+.2f}")

print()
print("-- Takeaway --")
print("   Same algorithm. Same split. MAE dropped just from features.")
print("   That's the 80/20 rule of ML: most gains come from features.")
