# ============================================
# CROSS VALIDATION — Full Solution
...
def stratified_kfold_cv(X, y, model_factory, k=5, seed=42):
    """Return numpy array of k fold-scores via stratified K-fold CV."""
    rng = np.random.default_rng(seed)
    y_int = np.asarray(y, dtype=int)

    # Stratified fold indices: split each class into k chunks
...
# Test with K=5 (or fewer folds if not enough rows per class)
...
# Demonstrate why one split lies
print()
print("=" * 60)
print("  COMPARISON: 5 single splits vs 5-fold CV")
print("=" * 60)
print("   Single splits (different seeds):")
from numpy.random import default_rng
for s in [1, 2, 3, 4, 5]:
    rng = default_rng(s)
    idx = rng.permutation(len(X))
    nt = max(1, int(0.2 * len(X)))
    te, tr = idx[:nt], idx[nt:]
    m = LogReg().fit(X[tr], y[tr])
    acc = m.score(X[te], y[te])
    print(f"     seed={s}: test_acc = {acc:.3f}")
print(f"\n   5-fold CV: {scores.mean():.3f} ± {scores.std():.3f}")
print("   The single-split spread is wider than ±2σ — single splits are noisy.")

print()
print("-- Takeaway --")
print("   One number = one sample. CV gives mean ± std — much more reliable.")
print("   A 0.01 difference within a 0.02 std is noise, not improvement.")
