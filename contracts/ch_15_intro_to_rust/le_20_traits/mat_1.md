...
So he did the obvious thing — `print_menu_summary(&MenuItem)` and `print_sales_summary(&DailySales)`. Two functions, identical job, identical shape, different parameter type — and a third already looming for receipts. Python Dan would have written *one* function and passed in anything with a `.summary()` method. Duck typing, bahala na. But a Rust function takes exactly one type, and Dan could feel the copy-paste future stretching ahead.

While stewing on that, he tried something small. The menu board printout shouldn't look like a debug dump, so he typed the most innocent line in programming — `println!("{}", adobo);` — and `cargo check` said no, politely and with a vocabulary word:

```text
error[E0277]: `MenuItem` doesn't implement `std::fmt::Display`
help: the trait `std::fmt::Display` is not implemented for `MenuItem`
   = note: in format strings you may be able to use `{:?}` instead
```

There it was again — that word. *Trait.* It had been showing up in error messages since Act 2, always in passing, like a name overheard at someone else's table. Not today. He called Kuya JM, who answered with a question about the serving rack.

> **Kuya JM:** Ilang ulam ang nakasalang?
>
> **Dan:** Five — adobo, sinigang, menudo, monggo, tortang talong.
>
> **Kuya JM:** Ilang sandok?
>
> **Dan:** ...Isa.
>
...
You have been implementing traits since Lesson 13. You just outsourced the typing:
