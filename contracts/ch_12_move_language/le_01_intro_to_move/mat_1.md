...
![Cover](https://bitdev-dml-assets.s3.ap-southeast-1.amazonaws.com/ch_12/le_01/cover.png)

## Scene

Neri stares at her monitor, the glow of error logs reflecting in her tired eyes. Another security audit, another list of vulnerabilities in their Solidity codebase. The payment system her team built for a local cooperative has been flagged for reentrancy risks—the same bug pattern that cost the industry billions.

"We keep patching symptoms," she mutters, pushing back from her desk in their Makati office.

Det, the team's senior engineer, walks over with two cups of kapeng barako. "I've been looking into something," he says, handing her a cup. "A language called Move. It was designed specifically to prevent the bugs we keep fighting."

Ronnie, overhearing from across the room, joins them. "Move? The one Facebook built for Diem?"

"Originally, yes," Det nods. "But it's evolved. Now it powers Sui, Aptos, Movement Labs—major blockchains. And here's the interesting part: the core language is the same across all of them. Learn Move once, deploy anywhere."

Neri takes a sip of her coffee. "Show me why it's different."

Det pulls up his laptop. "Let me introduce you to a language where safety isn't optional—it's built into every line of code."
...
This isn't a hypothetical problem. In the world of blockchain smart contracts, these kinds of bugs have cost billions of dollars.

> Move treats digital assets like physical objects—they can't be accidentally copied, deleted, or lost.

**The Problem with Traditional Smart Contract Languages:**

When you write a smart contract in Solidity, you're essentially working with a giant spreadsheet. Your tokens, your NFTs, your assets—they're all just numbers and data stored in this spreadsheet. Here's where things get dangerous:

**Reentrancy Attacks**: You write a function to withdraw money. The function checks your balance, sends you the money, then updates your balance to zero. But what if the receiver is a malicious contract that, when it receives the money, immediately calls your withdraw function again? Since your balance hasn't been updated yet, it checks your old balance and sends money again. And again. The infamous DAO hack of 2016 stole $60 million this way.

**Integer Overflow and Underflow**: Computers store numbers with a maximum size. If you have a `uint8` (which can hold 0-255), and you add 1 to 255, it doesn't become 256—it wraps around to 0! Imagine if your balance of 255 became 0 when someone deposited 1. These bugs have been exploited to create tokens out of thin air.

**Copy and Delete Bugs**: In traditional languages, you can accidentally duplicate or destroy value. You write `myTokens = 100; yourTokens = myTokens;` Now both variables hold 100—did you just create 100 tokens from nothing?

**Move's Revolutionary Approach:**

Now, imagine if digital assets worked like physical objects. You can't photocopy a gold bar. You can't accidentally delete your car keys. If you hand someone a peso bill, you no longer have it—it physically moved from your hand to theirs.
...
```move
// In traditional languages (dangerous):
// balance = 50; // Where did the original value go? Lost forever!
// No error. No warning. Funds just disappeared.

// In Move (safe):
let my_coin: Coin = get_coin();
// You MUST explicitly:
// - Transfer it: transfer(my_coin, recipient)
// - Destroy it: burn(my_coin) // Requires permission
// - Store it: move_to(account, my_coin)
// You cannot simply reassign and make it disappear!
```

Move's compiler acts like a strict guardian. It won't let you compile code that could accidentally lose assets.
