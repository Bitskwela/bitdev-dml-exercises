...
Loy, MoveStack's DevOps engineer, has set up a staging environment on the test network. Ronnie and Neri crowd around his monitor, watching deployment logs scroll past.

"Here's the problem," Loy explains, pointing at a failed transaction. "Your resources look perfect in memory. But resources have to *live* somewhere. They can't float in limbo. They need to be anchored to the chain."

Ronnie nods. "Global storage. The permanent home of all blockchain data."

"Exactly." Loy switches to a diagram showing addresses as boxes with compartments. "Every address is a vault. You store resources under addresses using move_to. Retrieve them with move_from. Read them with borrow_global. Modify them with borrow_global_mut."

Neri studies the screen. "So if I want to check if a user has a Config resource..."

"Use exists<Config>(address)," Loy says. "The chain tells you definitively—yes or no. No guessing, no null pointer exceptions."

"The key insight," Ronnie adds, "is type-indexed storage. Only one resource of each type per address. You can't store two Config objects under the same address."

Loy pulls up MoveStack's deployment script. "Today we build the configuration system. Store settings on-chain, retrieve them, update them. Data that persists forever—or until we explicitly remove it."

"Let's make it permanent," Neri says.
