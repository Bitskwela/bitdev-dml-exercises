...
The next morning arrives cold and clear. Odessa meets Kai, a seasoned spellcaster and the Guild's function architect, in the Crystal Tower's uppermost chamber. Kai is polishing an intricate staff, each rune glowing with purpose.

"You've learned how Move guards variables jealously," Kai says, gesturing for Odessa to sit. "Now, you'll learn why Move functions are like mathematical operations—pure, predictable, and composable."

Kai sets down the staff and fixes Odessa with an intense gaze. "In most lands, functions are wild beasts. They hide side effects, modify distant state, break when called twice in a row. But Move functions? They're like gears in a clock. You call them the same way every time, and they respond identically."

Kai walks to the window, pointing down at the city below. "Every transaction on the blockchain is a function call. If that function is unpredictable, the entire chain becomes unstable. But if every function is pure—deterministic, no hidden side effects—then the chain is unshakeable."

"Today, you'll master the anatomy of functions: their signatures, their purity, their visibility, and their power. By day's end, you'll write spells that can be called a thousand times and produce the same result every time."
